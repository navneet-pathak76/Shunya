...
class Habit {
  ...
  factory Habit.fromJson(Map<String, dynamic> j) => Habit(
    id: j['id'] as String? ?? '', name: j['name'] as String? ?? 'Habit',
    completedDates: (j['completedDates'] as List? ?? const []).map((x) => x.toString()).toList(),
  );
  bool completedOn(DateTime d) => completedDates.contains(_day(d));
  int get currentStreak {
    var d = DateTime.now(); var count = 0;
...
class HabitsState {
  ...
  int get completedToday => items.where((h) => h.completedOn(DateTime.now())).length;
}
...
class HabitsController extends StateNotifier<HabitsState> {
  HabitsController(this.ref) : super(const HabitsState()) { _load(); }
  final Ref ref; static const domain = 'habit';
  Future<void> _load() async {
    final r = await ref.read(localRecordRepositoryProvider.future);
    final list = (await r.listDomain(domain)).map((x) => Habit.fromJson(jsonDecode(x.payload) as Map<String,dynamic>)).toList();
    state = HabitsState(items: list);
  }
  Future<void> add(String name) async {
    final r = await ref.read(localRecordRepositoryProvider.future);
    final h = Habit(id: const Uuid().v4(), name: name.trim(), completedDates: const []);
    await r.upsert(domain: domain, key: h.id, payload: h.toJson(), recordDate: DateTime.now().toUtc()); await _load();
  }
  Future<void> toggle(Habit h) async {
    final today = Habit._day(DateTime.now()); final dates = [...h.completedDates];
    dates.contains(today) ? dates.remove(today) : dates.add(today);
    final r = await ref.read(localRecordRepositoryProvider.future);
    await r.upsert(domain: domain, key: h.id, payload: Habit(id:h.id,name:h.name,completedDates:dates).toJson(), recordDate: DateTime.now().toUtc()); await _load();
  }
